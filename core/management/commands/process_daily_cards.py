            ...
            for sub in birthdays:
                ...
                if agent.automation_mode == 'auto':
                    if sub.email:
                        ...
                        site_url = getattr(settings, 'SITE_URL', 'https://app.skandage.com')
                        image_url = f"{site_url}{template.image.url}" if template.image else ""
                        
                        context = {
                            'client_name': sub.name, 'agent': agent, 'occasion': 'Birthday',
                            'message': template.default_message, 'card_image_url': image_url, 'occasion_emoji': '🎂'
                        }
